...
/**
 * 
 * <tt>ProcessorA</tt> is used for manipulating incoming message by adding a
...
public class ProcessorA implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		System.out.println("*** entering ProcessorA.process");
		System.out.println(exchange.getIn().getBody());
		Payload payload = exchange.getIn().getBody(Payload.class);
		String message = payload.getMessage();
		System.out.println(message);
		payload.setMessage(message + " ProcessorA message");
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("name", "ProcessorA");
		map.put("successful", Boolean.TRUE);
		exchange.setProperty("hist-map", map);
		System.out.println("*** exiting ProcessorA.process");
	}
}
